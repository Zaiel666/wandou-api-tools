...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator("#lightboxCopy").isVisible(), true);
    assert.equal(await page.locator("#lightboxPrompt").isVisible(), true);
    assert.equal(await page.locator("#lightboxDownload").isVisible(), true);
    await page.locator("#lightboxImage").click({ button: "right" });
    await page.locator("#lightboxContextRegenerate").waitFor({ state: "visible" });
    assert.equal(await page.locator("#lightboxContextCopy").isVisible(), true);
    assert.equal(await page.locator("#lightboxContextPrompt").isVisible(), true);
    await page.locator("#lightboxImageMenu").evaluate((menu) => menu.classList.remove("open"));

...
assert.match(updater, /Directory\.Move\(install, previous\)/, "the mapped old install must be moved aside as one directory");
assert.match(updater, /Directory\.Move\(stage, install\)/, "the complete staged release must be activated as one directory");

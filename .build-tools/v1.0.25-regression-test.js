...
const checks = [
  [canvas.includes("const generationConcurrency = 3") && canvas.includes("await Promise.all(batch.map") && canvas.includes("batch.forEach((pending) => refreshRenderedNode(pending.id))"), "three-image concurrent generation queue"],
